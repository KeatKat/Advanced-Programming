...
//calling base class constructor to construct the object
Rectangle::Rectangle(string shape, bool containsWarpSpace)
: ShapeTwoD(shape,containsWarpSpace){
    readInCoords();
    calculateP();
    calculateA();
}
void Rectangle::readInCoords()
{   int xvalue,yvalue;
...
void Rectangle::getAllCoords(){
    for(int i=0;i<xValues.size();i++){
        cout << "Point [" << i << "]: (" << xValues.at(i) << ", " << yValues.at(i) << ")" << endl;
    }
}
void Rectangle::getPCoords(){
